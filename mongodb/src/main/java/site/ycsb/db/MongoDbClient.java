...
public class MongoDbClient extends DB {
  ...
  public Status insert(String table, String key,
    ...
    try {
      ...
      if(table.equals("jobs")) {
        ...
        String jobResult = new String(values.get("jobResult").toArray());
        toInsert = Document.parse(jobResult);
        toInsert.remove("jobId");
  ...
  public Pair findAndUpdate(String table, String key,
    ...
    try {
      ...
      if (version == null) {
        fieldsToSet.put("version", 1L);
      }
      Document update = new Document("$set", fieldsToSet);

      if (version != null) {
        update.append("$inc", new Document().append("version", 1L));
      }

      System.out.println("UPDATE DOC: " + update.toJson());
