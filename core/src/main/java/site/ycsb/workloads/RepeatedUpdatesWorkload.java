...
public class RepeatedUpdatesWorkload extends Workload {
  ...
  private final String[] TEST_JOB_IDS = new String[]{
  ...
  public boolean doTransaction(DB db, Object threadstate) {
    ...
    for (int i = 1; i <= 5 && status.isOk(); i++) {
      updatedValues.put("jobState", new ByteArrayByteIterator(Utils.longToBytes(i)));
      status = db.update("jobs", tracker.jobId, updatedValues);
    }
    final long endTime = System.nanoTime();
    Measurements.getMeasurements().measure("RAPID_UPDATE", (int) (startTime - endTime) / 1000);
    return status.isOk();
  }
}